...
public class ConduitNecklaceItem extends AbstractMementoItem {
    public static final String REGISTRY_NAME = "conduit_necklace";

    public ConduitNecklaceItem() {
        super(REGISTRY_NAME, getDefaultProps(ModStartupConfig.CONDUIT_NECKLACE.DURABILITY.get())
                .rarity(Rarity.UNCOMMON));
    }

    public boolean isEnabled() {
        return ModStartupConfig.CONDUIT_NECKLACE.ENABLED.get();
    }
    ...
    public void tick(ItemStack stack, SlotReference reference) {
        ...
        double radius = ModStartupConfig.CONDUIT_NECKLACE.RADIUS.get();
        double damage = ModStartupConfig.CONDUIT_NECKLACE.DAMAGE.get();
        int tickInterval = ModStartupConfig.CONDUIT_NECKLACE.DAMAGE_INTERVAL_TICKS.get();
        ...
        for (Entity mob : nearbyHostileAquaticMobs) {
            ...
            if (mob.tickCount > 0 && mob.tickCount % tickInterval == 0) {
                if (level.isClientSide()) {
                    ...
                    MobMementos.LOGGER.debug("[Conduit Necklace]: Damaging mob {}", mob);
                    ...
                    level.playLocalSound(mob.blockPosition(), ModSoundEvents.CONDUIT_NECKLACE_TRIGGERED.get(),
    ...
    private List<Entity> getNearbyHostileAquaticMobs(Level level, Entity entity, double radius) {
        return level.getEntities(entity, entity.getBoundingBox().inflate(radius), e -> {
            return e.getType().getTags()
                    .anyMatch(tag -> tag.location().toString().equals(MobMementos.MOD_ID + ":hostile_aquatic"))
                    && e instanceof Mob;
        });
    }
}
